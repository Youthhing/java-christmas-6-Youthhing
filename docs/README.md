...
- [x]  입력된 날짜 범위가 1~31이 아니면 예외를 발생시킨다.
...
- [x]  12월 1일부터 25일까지 지난 날짜를 계산하는 기능
...
- [x]  주문한 메뉴 목록에서 디저트의 개수를 계산한다.
- [x]  주문한 메뉴 목록에서 메인 메뉴의 개수를 계산한다.
...
- [x]  디데이, 평일 또는 주말, 특별할인 여부, 증정 여부에 따른 최종 혜택 금액을 계산한다.

### 할인 후 예상 결제 금액 계산 기능

- [x]  디데이할인, 평일, 주말할인, 특별할인을 통한 할인 후 예상 결제 금액을 계산한다.

### 증정 여부에 따른 할인 금액 계산 기능

- [x]  증정 여부에 따라 할인 금액을 계산한다. 증정하면 할인 금액에 25,000원을 포함한다.

### 평일, 주말여부에 따른 할인 금액 계산 기능

- [x]  평일이면 디저트 메뉴 1개당 2023원을 할인한다.
- [x]  주말이면 메인 메뉴 1개당 2023원을 할인한다.

### 특별 날짜 여부에 따른 할인 금액 계산 기능

- [x]  이벤트 날짜가 특별할인날이면 1000원을 할인한다.

### 구매 금액에 따른 할인 가능 여부 판단 기능

- [x]  총 주문 금액이 10,000원 이하이면 할인이 불가능하다.

### 배지 판별 기능

- [x]  할인 금액에 따라 배지를 판별한다.
